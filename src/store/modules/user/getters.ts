...
export const getters: GetterTree<State, RootState> & Getters = {
  ...
  [GettersType.GET_MODE]: (state) => state.userInfo.sideMode ?? 'dark',
  [GettersType.GET_BASE_COLOR]: (state) => state.userInfo.baseColor ?? '#4D70FF',
  [GettersType.GET_ACTIVE_COLOR]: (state) => state.userInfo.activeColor ?? '#fff',
